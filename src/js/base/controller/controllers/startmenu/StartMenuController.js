    ...
    cls.StartMenuController = context.oo.Class(cls.ControllerBase, function($super) {
      ...
      return {
        ...
        _getWidgetType: function(kind) {
          ...
          if (!kind) {
            if (this._widgetKind) {
              kind = this._widgetKind;
            } else {
              var windowNode = context.HostService.getCurrentWindow();
              if (windowNode) {
                kind = windowNode.getStartMenuType();
              }
            }
          }
        ...
        _createWidgetRecursive: function(node, kind) {
          ...
          currentController._widget = currentController._createWidget(type);
          var children = node.getChildren();
          for (var c = 0; c < children.length; c++) {
